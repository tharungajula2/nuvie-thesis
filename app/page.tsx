    ...
    <main className="min-h-screen selection:bg-accent selection:text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {siteContent.understanding.points.map((point, i) => (
              <div key={i} className="p-8 bg-background border border-foreground/5 rounded-2xl hover:border-accent/20 transition-all duration-500 group">
                ...
                <p className="text-foreground/45 font-light leading-relaxed text-sm">
                  {point.body}
                </p>
              </div>
            ))}
            ...
            <div className="md:w-2/3 space-y-12">
              {siteContent.usefulness.strengths.map((strength, i) => (
                <div key={i} className="relative pl-12">
                   ...
                   <p className="text-foreground/45 font-light leading-relaxed text-sm">{strength.body}</p>
                </div>
              ))}
            </div>
          </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Link href="/products" className="group">
                <div className="p-10 bg-background border border-foreground/5 rounded-3xl hover:border-accent/40 transition-all duration-500 h-full flex flex-col items-center text-center">
                   ...
                   <p className="text-xs text-foreground/40 font-light mb-8 flex-grow">A deep dive into the operational logic and sensory tradeoffs of the Nuvie portfolio.</p>
              ...
              <Link href="/occasions" className="group">
                <div className="p-10 bg-background border border-foreground/5 rounded-3xl hover:border-accent/40 transition-all duration-500 h-full flex flex-col items-center text-center">
                   ...
                   <p className="text-xs text-foreground/40 font-light mb-8 flex-grow">Mapping the 4 PM slump and the coffee ritual to distinct product wedges.</p>
              ...
              <Link href="/how-i-can-help" className="group">
                <div className="p-10 bg-background border border-foreground/5 rounded-3xl hover:border-accent/40 transition-all duration-500 h-full flex flex-col items-center text-center">
                   ...
                   <p className="text-xs text-foreground/40 font-light mb-8 flex-grow">Specific audit initiatives and growth frameworks translated into Nuvie goals.</p>
      ...
      <section className="py-48 px-6 bg-background relative overflow-hidden text-center">
        ...
        <div className="max-w-2xl mx-auto relative z-10">
           ...
           <p className="text-foreground/45 text-lg font-light mb-12 italic leading-relaxed">
