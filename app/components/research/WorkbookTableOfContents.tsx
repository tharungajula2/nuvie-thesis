  ...
  return (
    <>
        ...
        >
          ...
          <nav className="space-y-2">
            {chapters.map((chapter) => (
              ...
              >
                <span className="text-xs uppercase tracking-widest block mb-1 opacity-50">{chapter.eyebrow}</span>
                <span className="text-lg font-serif">{chapter.title}</span>
              </button>
            ))}
          </nav>
      ...
      <div className="hidden lg:flex flex-col h-full bg-graphite/5 rounded-[2.5rem] border border-foreground/5 p-8 relative overflow-hidden">
        <div className="mb-8">
          <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-foreground/20 block mb-2">Workbook</span>
          ...
          {chapters.map((chapter) => (
            ...
            >
              <div className="flex items-center gap-4">
                <span className={`text-[10px] font-mono transition-colors ${activeId === chapter.id ? 'text-accent' : 'text-foreground/20'}`}>
                  {chapters.indexOf(chapter) < 10 ? `0${chapters.indexOf(chapter)}` : chapters.indexOf(chapter)}
                </span>
                <span className={`text-sm tracking-tight transition-all ${activeId === chapter.id ? 'font-medium ml-1' : 'font-light'}`}>
                  {chapter.title}
                </span>
              </div>
            </button>
        ...
        <div className="mt-8 pt-6 border-t border-foreground/5">
          <button
            onClick={onLock}
            className="w-full text-left px-5 py-2 text-[8px] uppercase tracking-[0.4em] font-bold text-foreground/20 hover:text-accent transition-colors flex items-center gap-3"
          >
